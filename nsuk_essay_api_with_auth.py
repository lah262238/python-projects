...
if not st.session_state.logged_in:
    st.title("🔐 NSUK Essay Marking System")
    ...
    with tab2:
        ...
        account_type = st.radio(
            ...
            ["🎓 Student Account", "👨‍🏫 Lecturer Account", "👑 Admin Account"],
            horizontal=True,
            key="account_type"
        )
        ...
        else:  # Lecturer Account
            with col1:
                reg_id = st.text_input(
                    "Staff ID",
                    placeholder="e.g. STAFF/001 or LEC/2024/001",
                    key="reg_staff_id"
                )
                reg_name = st.text_input(
                    "Full Name",
                    placeholder="e.g. Dr. Abdullahi Hassan",
                    key="reg_staff_name"
                )
                reg_email = st.text_input(
                    "Official Email Address",
                    placeholder="e.g. lecturer@nsuk.edu.ng",
                    key="reg_staff_email"
                )
            with col2:
                reg_dept = st.text_input(
                ...
                reg_confirm = st.text_input(
                    "Confirm Password",
                    type="password",
                    key="reg_staff_confirm"
                )
            
            if st.button("Create Lecturer Account", type="primary", use_container_width=True):
                if reg_pass != reg_confirm:
                    st.error("Passwords do not match")
                elif reg_id and reg_name and reg_pass:
                    success, msg = auth.register_user(
                        reg_id, reg_name, reg_email,
                        reg_pass, "lecturer", reg_dept
                    )
                    st.success(msg) if success else st.error(msg)
                    if success:
                        st.info("Account created! Go to Login tab to sign in.")
...
# ============================================================
    ...
    st.markdown(f"**Welcome, {user['full_name']}! | Role: {user['role'].upper()}**")
    st.divider()
...
"""
    ...
    if user['role'] == 'student':
        ...
        if not active_exam:
            ...
            if student_already_submitted(exam_id, user['user_id']):
                st.warning("You have already submitted this exam.")
            else:
...
5. Conclusion (2 marks)""",
            ...
            if st.button("💾 Activate Exam", type="primary", use_container_width=True):
                if course_code and exam_question and marking_guide:
                    ...
                    cursor.execute("""
                        ...
                        VALUES (?, ?, ?, ?, ?, ?)
                    """, (course_code, course_title, user['full_name'], exam_question, marking_guide, max_score))
                    conn.commit()
        ...
        with col2:
            st.metric("Total Essays", total_results)
        with col3:
            st.metric("Approved", approved_results)
        with col4:
        ...
        tab1, tab2 = st.tabs(["📋 All Exams", "👥 User Management"])
